...
let GAS_WEBAPP_URL = 'https://script.google.com/macros/s/AKfycbxLKf0mS9H-u4qC98omFVOfmSdQQFhlx0C0qV19YayAUxtyVf03mFQnetkX1KKqZD1IaQ/exec';
...
async function displayBookingSlots() {
  ...
  try {
    ...
    TIME_SLOTS.day.slots.forEach(slot => {
      // スロットの開始時刻を計算
      const slotDateTime = new Date(createISODateTime(new Date(), slot.hour, slot.minute));
      // 5時間前の時刻を計算
      const bookingDeadline = new Date(slotDateTime.getTime() - 5 * 60 * 60 * 1000);
